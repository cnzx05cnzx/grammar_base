...
from utils.addtion_loss import CpoLoss, ECOPO_loss
...
class Seq2Labels(Model):
    ...
    def __init__(self, vocab: Vocabulary,
        ...
        self.loss_fct = ECOPO_loss(5)
        self.loss_fpt = CrossEntropyLoss(reduction='none')

        self.pinyin = Pinyin()
        self.sm_classifier = nn.Linear(768, self.pinyin.sm_size)
        self.ym_classifier = nn.Linear(768, self.pinyin.ym_size)
        self.sd_classifier = nn.Linear(768, self.pinyin.sd_size)
    ...
    def forward(self,  # type: ignore
                ...
                s_pinyin: torch.LongTensor = None,
                t_pinyin: torch.LongTensor = None,
        ...
        if labels is not None and d_tags is not None:
            # past mistake
            # loss_c = self.loss_fct(labels, logits_labels)

            # with pinyin
            encoded_pinyin = self.text_field_embedder(s_pinyin)
            sm_scores = self.sm_classifier(encoded_pinyin)
            ym_scores = self.ym_classifier(encoded_pinyin)
            sd_scores = self.sd_classifier(encoded_pinyin)

            loss_fct = self.loss_fpt  # -100 index = padding token

            if t_pinyin is not None:
                t_pinyin = t_pinyin.long()
                active_loss = mask.view(-1) == 1
                active_labels = torch.where(
                    active_loss, t_pinyin[..., 0].view(-1),
                    torch.tensor(loss_fct.ignore_index).type_as(t_pinyin)
                )
                sm_loss = loss_fct(sm_scores.view(-1, self.pinyin.sm_size), active_labels)
                active_labels = torch.where(
                    active_loss, t_pinyin[..., 1].view(-1),
                    torch.tensor(loss_fct.ignore_index).type_as(t_pinyin)
                )
                ym_loss = loss_fct(ym_scores.view(-1, self.pinyin.ym_size), active_labels)
                active_labels = torch.where(
                    active_loss, t_pinyin[..., 2].view(-1),
                    torch.tensor(loss_fct.ignore_index).type_as(t_pinyin)
                )
                sd_loss = loss_fct(sd_scores.view(-1, self.pinyin.sd_size), active_labels)
                phonetic_loss = (sm_loss + ym_loss + sd_loss) / 3
                phonetic_loss = torch.mean(phonetic_loss)
            ...
            output_dict["loss"] = loss_labels + loss_d + phonetic_loss
