...
class GecBERTModel(object):
    ...
    def predict(self, batches):
        ...
        for batch, model in zip(batches, self.models):
            batch = util.move_to_device(batch.as_tensor_dict(),
                                        0 if torch.cuda.is_available() and self.use_cpu == False else -1)
            with torch.no_grad():
    ...
    # new
    def check_batch(self, b_batch, o_batch, f_batch, pos):

        res = 0
        for orig_id in pos:
            # print(o_batch[orig_id])
            # print(f_batch[orig_id])
            b, o, f = b_batch[orig_id], o_batch[orig_id], f_batch[orig_id]
            n = len(b)
            if o == f:
                continue
            else:
                for i, b_word, o_word, f_word in enumerate(zip(b, o, f)):
                    if b_word != o_word:
                        l, mid, r = max(i - 1, 0), i, min(i + 1, n - 1)
                        if f[l] == o[l] and f[mid] == o[mid] and f[r] == o[r]:
                            continue
                        else:
                            o[i] = b[i]
                            res += 1
    ...
    def handle_batch(self, full_batch):
        ...
        # new
        before_batch, one_batch, one_ids, one_cnt = final_batch, final_batch, pred_ids, total_updates
        ...
        down = 0
        if self.check:
            final_batch, down = self.check_batch(before_batch, one_batch, final_batch, one_ids)

        return final_batch, one_cnt - down
