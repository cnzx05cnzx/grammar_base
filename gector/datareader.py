...
from utils.pinyin import hanzi2pinyin
...
class Seq2LabelsDatasetReader(DatasetReader):
    ...
    def __init__(self,
        ...
        self.tokenizer = BertWordPieceTokenizer('data/vocab.txt', lowercase=True)
        self.token2pinyin = hanzi2pinyin('data')
    ...
    def text_to_instance(self, tokens: List[Token], tags: List[str] = None,
        ...
        if tags is not None:
            ...
            tmp = (['s'] + words[1:])[:self._max_len]
            tar = ''
            s_pinyin = self.token2pinyin.convert_sentence_to_pinyin(tmp)
            ...
            t_pinyin = self.token2pinyin.convert_sentence_to_s_y_s_ids(tar)
            # s_pinyin = [
            #     self.token2pinyin.pinyin2tensor[x] if x in self.token2pinyin.pinyin2tensor else [0, 0, 0, 0, 0, 0, 0, 0]
            #     for x in s_pinyin]
            # t_pinyin = [
            #     self.token2pinyin.pinyin2tensor[x] if x in self.token2pinyin.pinyin2tensor else [0, 0, 0, 0, 0, 0, 0, 0]
            #     for x in t_pinyin]
            ...
            # fields["s_pinyin"] = ArrayField(np.array(s_pinyin))
            fields["s_pinyin"] = TextField(s_pinyin, self._token_indexers)
            fields["t_pinyin"] = ArrayField(np.array(t_pinyin))
