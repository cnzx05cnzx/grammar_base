...
def new_Matcher(source, target):
    res = []
    pos, tmp, n = 0, 0, len(source)
    while pos < n:
        if source[tmp] == target[tmp]:
            while tmp < n and source[tmp] == target[tmp]:
                tmp += 1
            res.append(('equal', pos, tmp, pos, tmp))
        else:
            res.append(('replace', pos, tmp + 1, pos, tmp + 1))
            tmp += 1
        pos = tmp
    return res
...
def align_sequences(source_sent, target_sent):
    ...
    diffs = new_Matcher(source_tokens, target_tokens)
    all_edits = []
    for diff in diffs:
